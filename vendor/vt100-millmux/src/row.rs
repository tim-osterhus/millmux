...
impl Row {
    ...
    pub fn resize(&mut self, len: u16, cell: crate::Cell) {
        if usize::from(len) < self.cells.len() {
            ...
            let restore_wide_continuation = self.cells.len() == 1 && self.cells[0].is_wide();
            self.cells.resize(usize::from(len), cell);
            if restore_wide_continuation && self.cells.len() > 1 {
                self.cells[1].set_wide_continuation(true);
            }
            self.sanitize_wide_cells();
        }
        self.wrapped = false;
    ...
    fn sanitize_wide_cells(&mut self) {
        for index in 0..self.cells.len() {
            let valid = if self.cells[index].is_wide() {
                (self.cells.len() == 1 && index == 0)
                    || self
                        .cells
                        .get(index + 1)
                        .is_some_and(crate::Cell::is_wide_continuation)
            } else if self.cells[index].is_wide_continuation() {
                index > 0 && self.cells[index - 1].is_wide()
            } else {
                true
            };
            if !valid {
                let attrs = *self.cells[index].attrs();
                self.cells[index].clear(attrs);
            }
        }
    }
    ...
    pub fn write_contents_formatted(
            ...
            .take(usize::from(width))
        {
            ...
            if let Some((prev_col, attrs)) = erase {
                if cell.is_occupied() || cell.attrs() != attrs {
...
mod tests {
    ...
    #[test]
    fn resize_preserves_only_valid_wide_lead_states() {
        let mut one_column = Row::new(1);
        one_column.get_mut(0).unwrap().set_grapheme(
            "\u{754c}",
            true,
            crate::attrs::Attrs::default(),
        );
        assert!(one_column.get(0).unwrap().is_wide());
        assert!(one_column.get(1).is_none());

        one_column.resize(2, crate::Cell::new());
        assert!(one_column.get(0).unwrap().is_wide());
        assert!(one_column.get(1).unwrap().is_wide_continuation());

        let mut truncated = Row::new(3);
        truncated.get_mut(1).unwrap().set_grapheme(
            "\u{754c}",
            true,
            crate::attrs::Attrs::default(),
        );
        truncated.get_mut(2).unwrap().set_wide_continuation(true);
        truncated.resize(2, crate::Cell::new());
        assert!(!truncated.get(1).unwrap().is_wide());
        assert!(!truncated.get(1).unwrap().is_wide_continuation());
        assert!(!truncated.get(1).unwrap().has_contents());
    }
}
