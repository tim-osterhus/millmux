...
/// Non-rendered drawing attributes retained by a parser checkpoint.
#[derive(Default, Clone, Copy, PartialEq, Eq, Debug)]
pub struct AttrsContinuation {
    /// Foreground color selected for subsequent output.
    pub fgcolor: Color,
    /// Background color selected for subsequent output.
    pub bgcolor: Color,
    /// Encoded text-mode flags.
    pub mode: u8,
}

impl From<Attrs> for AttrsContinuation {
...
impl From<AttrsContinuation> for Attrs {
    fn from(continuation: AttrsContinuation) -> Self {
        Self {
            fgcolor: continuation.fgcolor,
            bgcolor: continuation.bgcolor,
