...
/// Non-rendered grid state retained by a parser checkpoint.
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub struct GridContinuation {
    /// Current drawing cursor.
    pub pos: Pos,
    /// DEC-saved drawing cursor.
    pub saved_pos: Pos,
    /// Inclusive top of the scroll region.
    pub scroll_top: u16,
    /// Inclusive bottom of the scroll region.
    pub scroll_bottom: u16,
    /// Whether origin mode is active.
    pub origin_mode: bool,
    /// Origin mode stored with the saved cursor.
...
impl Grid {
    ...
    pub fn set_size(&mut self, size: Size) {
        ...
        if columns_changed && self.saved_pos.col > self.size.cols - 1 {
    ...
    pub(crate) fn continuation(&self) -> GridContinuation {
        GridContinuation {
            pos: self.pos,
    ...
    pub(crate) fn restore_continuation(&mut self, continuation: GridContinuation) {
        let last_row = self.size.rows.saturating_sub(1);
        self.scroll_top = continuation.scroll_top.min(last_row);
        self.scroll_bottom = continuation.scroll_bottom.min(last_row);
        if self.scroll_top >= self.scroll_bottom {
            self.scroll_top = 0;
            self.scroll_bottom = last_row;
        }
        self.pos = Pos {
            row: continuation.pos.row.min(last_row),
            col: continuation.pos.col.min(self.size.cols),
        };
        self.saved_pos = Pos {
            row: continuation.saved_pos.row.min(last_row),
            col: continuation.saved_pos.col.min(self.size.cols),
        };
        self.origin_mode = continuation.origin_mode;
        self.saved_origin_mode = continuation.saved_origin_mode;
    }
    ...
    pub fn col_tab(&mut self) {
        let start = self.pos.col;
        self.pos.col -= self.pos.col % 8;
        self.pos.col += 8;
        self.col_clamp();
        let end = self.pos.col;
        let row = self.current_row_mut();
        for col in start..end {
            row.get_mut(col).unwrap().mark_occupied_blank();
        }
    }
...
/// A zero-based terminal grid position.
#[derive(Copy, Clone, Debug, Default, Eq, PartialEq)]
pub struct Pos {
...
mod tests {
    ...
    #[test]
    fn restored_continuation_preserves_right_margin_wrap_pending_and_saved_cursor() {
        let size = Size { rows: 2, cols: 4 };
        let mut continuous = Grid::new(size, 0);
        continuous.allocate_rows();
        continuous.pos = Pos { row: 0, col: 4 };
        continuous.saved_pos = Pos { row: 0, col: 4 };
        let checkpoint = continuous.continuation();

        let mut restored = Grid::new(size, 0);
        restored.allocate_rows();
        restored.restore_continuation(checkpoint);
        assert_eq!(restored.pos(), continuous.pos());
        restored.restore_cursor();
        assert_eq!(restored.pos(), continuous.pos());

        continuous.col_wrap(1, true);
        restored.col_wrap(1, true);
        assert_eq!(restored.pos(), continuous.pos());
    }

    #[test]
    fn row_only_resize_preserves_wrap_pending_current_and_saved_cursor() {
        let mut grid = Grid::new(Size { rows: 2, cols: 4 }, 0);
        grid.allocate_rows();
        grid.pos = Pos { row: 0, col: 4 };
        grid.saved_pos = Pos { row: 0, col: 4 };

        grid.set_size(Size { rows: 3, cols: 4 });
        assert_eq!(grid.pos, Pos { row: 0, col: 4 });
        grid.restore_cursor();
        assert_eq!(grid.pos, Pos { row: 0, col: 4 });
