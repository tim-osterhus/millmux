...
impl<CB: crate::callbacks::Callbacks> WrappedScreen<CB> {
    ...
    pub(crate) fn completed_dispatches(&self) -> u64 {
        self.completed_dispatches
    }

    fn completed_dispatch(&mut self) {
...
impl<CB: crate::callbacks::Callbacks> vte::Perform for WrappedScreen<CB> {
    ...
    fn execute(&mut self, b: u8) {
        match b {
            ...
            _ => {
                self.screen.break_grapheme();
                self.callbacks.unhandled_control(&mut self.screen, b);
            }
        }
    }
    ...
    fn csi_dispatch(&mut self, params: &vte::Params, intermediates: &[u8], _ignore: bool, c: char) {
        self.completed_dispatch();
        if !grapheme_neutral_csi(params, intermediates, c) {
            self.screen.break_grapheme();
        }
...
fn grapheme_neutral_csi(params: &vte::Params, intermediates: &[u8], c: char) -> bool {
    if intermediates.is_empty() && c == 'm' {
        return true;
    }
    ...
    params.iter().all(|param| {
        matches!(
            param,
            [1 | 9 | 25 | 1000 | 1002 | 1003 | 1005 | 1006 | 2004 | 2026]
        )
    })
}
